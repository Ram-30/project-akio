...
from nextcord_components import nextcordComponents, Button
from nextcord_slash import cog_ext, SlashContext
...
class Image(commands.Cog):
    ...
    async def gay(self, ctx, user=None):
        if not user:
            user = ctx.author
        # this is a example, everything is optional.
        embed = nextcord.Embed(title=f"{user.name}", color=ctx.author.color)
        # attaching file image to embed.
        embed.set_image(url="attachment://gay.png")
    # Wrapper
        try:
            # get Image object
            image = await self.bot.alex_api.filter(13, user.avatar_url)
            image_bytes = await image.read()  # get io.BytesIO object
         # Sending
            # pass io.BytesIO object to nextcord.File with a filename.
            file = nextcord.File(image_bytes, "gay.png")
            # send both the embed and file, the file will attach to the embed.
    ...
    async def achievement(self, ctx, *, text: str, icon=None):
        # this is a example, everything is optional.
        embed = nextcord.Embed(
            title=f"{ctx.author} unlocked a new achievement", color=ctx.author.color)
        # attaching file image to embed.
        embed.set_image(url="attachment://gay.png")
    # Wrapper
    ...
    async def vs(self, ctx, member1: nextcord.Member, member2: nextcord.Member):
        member1 = member1.avatar_url_as(
            size=1024, format=None, static_format='png')
        member2 = member2.avatar_url_as(
            size=1024, format=None, static_format='png')
    ...
    async def magik(self, ctx, user=None, intensity: int = 5):
        ...
        try:
            ...
            res = await r.json()
            embed = nextcord.Embed(color=self.bot.embed_color, title="→ Magik")
            embed.set_image(url=res["message"])
    ...
    async def deepfry(self, ctx, user=None):
        ...
        try:
            ...
            res = await r.json()
            embed = nextcord.Embed(
                color=self.bot.embed_color, title="→ deepfry")
            embed.set_image(url=res["message"])
    ...
    async def youtube(self, ctx, *, comment):
        picture = ctx.author.avatar_url_as(
            size=1024, format=None, static_format='png')
        ...
        youtube_file = nextcord.File(res, filename=f"youtube.jpg")
        embed = nextcord.Embed(
            color=self.bot.embed_color,
